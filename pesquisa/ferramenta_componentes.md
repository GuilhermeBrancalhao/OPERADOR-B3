...
| **Renko A1/A2/A3 + cor interna** | `fluxopro/analytics/renko.py` (26/08/2026) — tijolos por deslocamento, fase (TENDENCIA/PERDENDO_FORCA/POSSIVEL_INVERSAO) e alvos A1/A2/A3 por amplitude de janela recente. Desenhado no workspace OPERADOR B3 (`nexo/forca.py`, região "4R"). | **Construído**, com os limites já documentados no módulo: tamanho de tijolo (4 pontos) e limiar de tijolos-para-tendência são parâmetros do operador, não da fonte (IMPRECISO); a amplitude que ancora A1/A2/A3 é proxy por alcance de janela, não a "amplitude média da frequência do dia" do autor (fórmula exata segue AUSENTE NA FONTE). |
