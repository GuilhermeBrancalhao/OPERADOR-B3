...
nao ha lookahead e nao ha liquidez sintetizada. Preco chega em ``int`` de
ticks e vira pixel apenas aqui, na fronteira de desenho (``y_preco`` e
``formato.preco_completo``); nada volta a ser gravado no snapshot.
...
def desenhar(painter: QPainter, rect: QRect, estado: EstadoNexo) -> None:
    ...
    # de um plot largo, senao cada vela vira uma lasca.
